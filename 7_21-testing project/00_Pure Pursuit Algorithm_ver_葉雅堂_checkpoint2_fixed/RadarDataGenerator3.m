...
while(looping < 3 )  %Running how many loops of the codes
restart(scenario);
...
%Curvatives Adjusting speed
 if norm(D_vector,2) < Car_Velocity*7 %Curvatives > Certain amount changing speed
   
     velocity=sqrt(Car_Velocity*Car_Velocity + 2*a*D_ForR);
     display(velocity);
     
     if Lane_Index == 1    %Changing lane
        velocity = 120;
     end
 else
